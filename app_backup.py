...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///socrates.db'
...
def dashboard():
    ...
    if is_admin:
        ...
        # Produtor vê apenas seus eventos
...
def listar_eventos():
    ...
    print(f"Data limite (90 dias atrás): {limite_passado}")
    ...
    eventos_query = Evento.query.filter(
        ...
        # Mostrar eventos dos últimos 90 dias em diante (inclui eventos futuros)
...
# Endpoints de API para carregamento dinâmico nos modais
...
def salvar_receita_individual(id_evento):
    try:
        ...
        print(f"✅ Receita salva: {receita_nome} - R$ {valor:,.2f}")
...
def novo_evento():
    ...
    # Filtrar apenas categorias que têm despesas de evento (tipos 1 e 2)
    categorias_despesa = CategoriaDespesa.query.filter(
        CategoriaDespesa.id_categoria_despesa.in_(
            db.session.query(Despesa.id_categoria_despesa).filter(
                Despesa.id_tipo_despesa.in_([1, 2])
    ...
    if form.validate_on_submit():
        ...
        db.session.add(novo)
        print(f"Evento adicionado à sessão")
        ...
        try:
            db.session.commit()
            print(f"✅ COMMIT executado com sucesso! Evento ID: {novo.id_evento}")
            
            # Verificar se o evento foi realmente salvo
            evento_verificacao = Evento.query.get(novo.id_evento)
            if evento_verificacao:
                print(f"✅ Evento verificado no banco: {evento_verificacao.nome}")
            else:
                print("❌ ERRO: Evento não encontrado no banco após commit!")
            
            # Verificar as despesas APÓS o commit
    ...
    # Para a exibição inicial (antes de salvar), mostrar todas as despesas disponíveis
    ...
    # Criar estrutura de despesas organizada por tipo para exibição
...
def editar_evento(id):
    ...
    if form.validate_on_submit():
        ...
        for i in range(len(despesa_ids)):
            try:
                # Verificar se tem ID da despesa (obrigatório)
                if not despesa_ids[i]:
                    print(f"Índice {i}: ID da despesa vazio, pulando...")
                    continue
                    
                # Verificar se tem valor preenchido
                if not valores_desp[i] or valores_desp[i].strip() == '':
                    print(f"Índice {i}: Valor da despesa vazio, pulando...")
                    continue
        ...
        try:
            db.session.commit()
            print(f"✅ COMMIT executado com sucesso! Evento ID: {evento.id_evento}")
    ...
    # Criar estrutura de despesas para edição - mostrar apenas as que faltam cadastrar
    categorias_despesa_dict = {}
    for categoria in categorias_despesa:
...
def relatorios_lucratividade_periodo():
    ...
    # Filtro de datas para período de análise
    period = request.args.get('period', 'ano')
    if period == 'mes':
        # Últimos 12 meses
        data_fim = date.today().replace(day=1)  # Primeiro dia do mês atual
        data_inicio = (data_fim - timedelta(days=365)).replace(day=1)  # 12 meses atrás
    elif period == '6meses':
        # Últimos 6 meses
    ...
    end_date = data_fim.replace(day=1)  # Primeiro dia do mês final
...
def relatorios_faturamento_evento():
    ...
    # Calcular lucratividade dos eventos para o gráfico
...
def excluir_veiculo(id):
    veiculo = Veiculo.query.get_or_404(id)
    # Veículos normalmente não têm dependências diretas no sistema atual
    # Mas se houver tabelas de relacionamento no futuro, adicionar aqui
...
def salvar_despesa_individual(id_evento):
    ...
    try:
        ...
        print(f"É despesa fixa: {is_fixa}")
        ...
        # Verificar se a despesa já existe para este evento
        ...
        if despesa_existente:
            ...
            print(f"✅ Atualizando despesa existente ID {despesa_existente.id_despesa_evento}")
            print(f"   Valor antigo: {despesa_existente.valor} → Valor novo: {valor_float}")
            ...
            print(f"✅ Despesa atualizada com sucesso!")
            ...
            # CRIAR nova despesa no evento (comum para despesas variáveis)
